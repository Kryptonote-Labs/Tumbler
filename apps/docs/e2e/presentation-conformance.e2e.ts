...
test('Windows metafiles decode lazily into SVG pictures',async({page})=>{
 const data=new Uint8Array(132),v=new DataView(data.buffer);const put=(offset:number,...values:number[])=>values.forEach((value,i)=>v.setUint32(offset+i*4,value,true));
 put(0,1,88,0,0,100,100,0,0,2540,2540,0x464d4520,0x10000,132,3);v.setUint16(56,1,true);put(72,100,100,26,26);put(88,43,24,10,10,90,90);put(112,14,20,0,0,20);
 const pkg=openOpcPackage(new Uint8Array(await readFile(fixture))),tx=beginPackageTransaction(pkg),part=pkg.getPart('/ppt/slides/slide1.xml')!;
 tx.addPart('/ppt/media/test.emf','image/x-emf',data);tx.addRelationship(part.name,{id:'metafile',type:'http://schemas.openxmlformats.org/officeDocument/2006/relationships/image',target:'/ppt/media/test.emf'});
 const xml=new TextDecoder().decode(pkg.readPart(part)).replace('<p:spPr>','<p:spPr><a:blipFill><a:blip r:embed="metafile"/><a:stretch><a:fillRect/></a:stretch></a:blipFill>');
 tx.replacePart(part.name,new TextEncoder().encode(xml));
 await page.goto('/playground/slides-brief');await expect(page.locator('.slide-stage').getByText('A quieter workspace',{exact:true}).last()).toBeVisible();
 await page.locator('input[type=file]').setInputFiles({name:'metafile.pptx',mimeType:'application/vnd.openxmlformats-officedocument.presentationml.presentation',buffer:Buffer.from(tx.commit())});
 const image=page.locator('.slide-stage pattern image');await expect(image).toHaveAttribute('href',/^blob:/);
 const svg=await image.evaluate(async e=>fetch(e.getAttribute('href')!).then(r=>r.text()));expect(svg).toContain('<svg');expect(svg).toContain('rect');
});
