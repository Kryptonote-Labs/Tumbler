import {cartesianStack, chartValueCoordinate, layoutCartesianChart} from './layout.ts';
...
export function layoutCombinationChart(model:SupportedChartModel,width:number,height:number) {
 ...
 return plots.map((plot,i)=>{
   const axis=plot.axes.find(a=>a.kind==='value' && !a.deleted && (!plot.axisIds || plot.axisIds.includes(a.id)));
   const peers=plots.flatMap((p,j)=> (p.axisIds?.at(-1)===plot.axisIds?.at(-1))?[layouts[j]!]:[]);
   const minimum=axis?.minimum??Math.min(...peers.map(l=>l.minimum));
   const maximum=axis?.maximum??Math.max(...peers.map(l=>l.maximum));
   const coordinate=(value:number)=>chartValueCoordinate(value,minimum,maximum,plot.kind==='bar'?rect.x:rect.y,plot.kind==='bar'?rect.width:rect.height,plot.kind!=='bar');
   return {model:plot,axis,minimum,maximum,plot:rect,categories:common.categories,coordinate,
     points:plot.series.map((_,s)=>common.categories.map((_,c)=>{const point=cartesianStack(plot,s,c);return point?{start:coordinate(point.start),end:coordinate(point.end)}:undefined;}))};
 });
}
