...
export function embeddedFontBytes(bytes: Uint8Array): Uint8Array | undefined {
  const signature = (data: Uint8Array) =>
    data.length >= 4 &&
    [0x00010000, 0x4f54544f, 0x74727565, 0x774f4646, 0x774f4632].includes(
      new DataView(data.buffer, data.byteOffset, data.byteLength).getUint32(0),
    );
  ...
  if (
    size !== bytes.length ||
    length > size - 82 ||
    view.getUint16(34, true) !== 0x504c ||
    bytes.length > 16 * 1024 * 1024
  )
    return;
  if ((flags & 4) !== 0)
    try {
      const font = eotToTtf(bytes);
      return font.length <= 64 * 1024 * 1024 && signature(font)
        ? font
        : undefined;
    } catch {
      return;
