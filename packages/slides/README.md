...
Run `bun run compatibility:presentation-rendering` to regenerate `rendering-checks.pptx`. Full timing-tree execution, media synchronization, reflections, 3D lighting/bevels, and complete WordArt/vertical typography are still outstanding. Preview tests are not an Office interoperability certification.
