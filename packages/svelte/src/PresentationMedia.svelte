<script lang="ts">
  ...
  let {
    ...
    media: NonNullable<SlideObject["media"]>;
    poster?: string;
    active?: boolean;
  } = $props();
  let url = $state(""),
  ...
  function loaded() {
    if (player && media.start) player.currentTime = media.start;
  }
  function update() {
    if (player && media.end !== undefined && player.currentTime >= media.end)
      player.pause();
  }
  $effect(() => {
...
<div
  ...
  role="group"
  aria-label={`${media.kind} playback`}
>
  {#if !source && media.url}<button onclick={() => (allowed = true)}
      >Load linked {media.kind}</button
    >
